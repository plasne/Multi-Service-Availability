...
module.exports = {
    ...
    start: function(context) {
        ...
        context.events.on("rule.timeout:elapsed", change);

        // allow rule processing every so often
        const every = context.region["process-rules-every"];
        if (every) {
            setInterval(function() {
                context.events.emit("rule.timeout:elapsed");
            }, every);
        }
    ...
    evaluate: function(context) {
        ...
        context.services.forEach(function(service) {
            console.log("service.pre-process: " + service.name);
            service.preprocess(context);
        });
