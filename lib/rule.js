...
module.exports = {
    ...
    new: function(base) {
        ...
        base.evaluate = function(context) {
            ...
            if (global.loglevel >= 4) console.log("rule (%s) will attempt to resolve the if clause (%s).", base.name, base.if);
            var formula = base.resolve(context);
            if (global.loglevel >= 4) console.log("rule (%s) resolved the if clause (%s).", base.name, formula);
            try {
    ...
    validate: function(context) {
        ...
        me.rules.forEach(function(rule) {
            ...
            try {
                var value = eval(formula);
                if (global.loglevel >= 4) console.log("the rule (%s) formula (%s) was successfully evaluated (%s) as (%s).", rule.name, rule.if, formula, value);
            } catch (ex) {
