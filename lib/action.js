    ...
    new: function(base, rule, clause) {
        ...
        if (base.type == "update") {
            ...
            if (count > 0) {
                ...
                base.execute = function(context) {
                    ...
                    context.services.forEach(function(service) {
                        if (service.isLocal && (is_wildcard || base.services.indexOf(service.name) > -1)) {
                            ...
                            if (base.priority) {
                                const priority = service.priorities.find(function(p) { return (p.rule == rule.name); });
                                if (priority) {
                                    priority.value = base.priority;
                                } else {
                                    service.priorities.push({ rule: rule.name, value: base.priority });
                                }
                            }
                ...
                throw new verror("10611: an update action must have at least one state, report, or priority node.");
            }
                    ...
                    try {
                        ...
                        request(base, function(error, response, body) {
                            if (!error && response.statusCode == 200) {
                                ...
                                console.error(new verror(error, "rule (%s) clause (%s) webhook (%s) got an error.", rule.name, clause, base.uri));
                            }
                        });
                    } catch (ex) {
                        console.error(new verror(ex, "rule (%s) clause (%s) webhook (%s) got an exception.", rule.name, clause, base.uri));
                    }
                    base.isExecuting = false;
                } else {
                    if (global.msa_settings.loglevel >= 4) console.log("rule (%s) clause (%s) skipped executing the webhook (%s) because it was still in progress from a prior execution.", rule.name, clause, base.uri)
                }
            }
