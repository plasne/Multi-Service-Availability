...
module.exports = {
    ...
    validate: function(context, argv) {
        const me = this;

        // ensure an instance is specified
        if (argv.instance == null) {
            throw new verror("10361: the instance must be specified.");
        }
                ...
                if (region.instances.length > 0) {
                    const elect = function() {
                        try {
                                ...
                                if (instance != region.instance) {
                                    ...
                                    instance.elect.execute().then(function(result) {
                                        if (global.loglevel >= 4) {
                                            console.log("ELECTION RESPONSE FROM %s:", instance.elect.uri);
                            ...
                            throw new verror(ex, "10371: unhandled exception on election.");
                        }
                        ...
                        if (me.region.instance.isMaster) {
                            try {
                                instance.query.execute().then(function(result) {
                                    ...
                                    instance.isConnected = true;
                                    if (global.loglevel >= 4) {
                                        console.log("QUERY RESPONSE FROM %s:", instance.query.uri);
                                        console.log(result.body);
                                    } else {
                                        console.log("querying %s for remote status succeeded.", instance.name);
                                    }

// WHY IS THIS NOT parsed AND ELECTION IS?
                                ...
                                throw new verror(ex, "10381: unhandled exception on query.");
                            }
                        }
