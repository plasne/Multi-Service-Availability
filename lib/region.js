            ...
            dns.lookup(global.msa_settings.discover_dns, {
                ...
                if (!error) {
                    if (addresses.length > 0) {
                        ...
                        console.error("10003: the discover DNS entry (%s) did not find any addresses.", discover_dns);
                        deferred.reject(discovered_instances);
                    }
                } else {
                    // log as error but don't stop execution
                    console.error(new verror(error, "10004: the discover DNS entry (%s) could not be resolved.", discover_dns).message);
                    deferred.reject(discovered_instances);
                }
            });
        } else {
                ...
                if (region.instances.length > 0) {
                    const elect = function() {
                        try {
                            ...
                            me.discover().done(function(discovered_instances) {
                                console.log("1: count = %d", discovered_instances.length);
                                if (Array.isArray(discovered_instances)) {
                                    ...
                                    discovered_instances.forEach(function(di) {
                                        ...
                                        if (!found) {
                                            me.region.instances.push(instance_manager.new(found));
                                            found.isRemote = true;
                                            found.isLocal = false;
                                            console.info("the DNS discovery process added a new instance (%s).", found.name);
                                        }
                                    });
                            ...
                            throw new verror(ex, "10372: unhandled exception on region.elect (%s).", region.name);
                        }
                    ...
                    const poll = function() {
                        if (me.region.instance.isMaster) {
                            try {
                                ...
                                throw new verror(ex, "10373: unhandled exception on region.query (%s).", region.name);
                            }
                        }
